program euler_sist_solar !Codi per efectuar la simulació del sistema solar en el pla usant el mètode d'Euler. El programa està preparat per poder simular tots els cossos, però per simplicitat ens quedarem només amb el sistema Mercuri, Venus, Terra, Mart, Júpiter (i Sol, naturalment).
    ...
    open(unit=10, file='results_euler100anys.dat', status='replace')
    open(unit=20, file='sol100anys.dat', status='replace')
        write(20,*) rx(1), ry(1)
    open(unit=30, file='mercuri100anys.dat', status='replace')
        write(30,*) rx(2), ry(2)
    open(unit=40, file='venus100anys.dat', status='replace')
        write(40,*) rx(3), ry(3)
    open(unit=50, file='terra100anys.dat', status='replace')
        write(50,*) rx(4), ry(4)
    open(unit=60, file='mart100anys.dat', status='replace')
        write(60,*) rx(5), ry(5)
    open(unit=70, file='jupiter100anys.dat', status='replace')
        write(70,*) rx(6), ry(6)
